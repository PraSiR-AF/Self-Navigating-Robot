...
int main(int argc, char *argv[]) {
    ...
    //OUR MAZE!!!
    /*insertAndSetFirstWall(&head, 1,  OVERALL_WINDOW_WIDTH/2, OVERALL_WINDOW_HEIGHT/2+50, 10, 100);
    insertAndSetFirstWall(&head, 2,  OVERALL_WINDOW_WIDTH/2-200, OVERALL_WINDOW_HEIGHT/2+50, 10, OVERALL_WINDOW_HEIGHT/2+50);
    insertAndSetFirstWall(&head, 3,  OVERALL_WINDOW_WIDTH/2, OVERALL_WINDOW_HEIGHT/2+100, 110, 10);
    insertAndSetFirstWall(&head, 4,  OVERALL_WINDOW_WIDTH/2-200, OVERALL_WINDOW_HEIGHT/2+50, 200, 10);
    insertAndSetFirstWall(&head, 5,  OVERALL_WINDOW_WIDTH/2, OVERALL_WINDOW_HEIGHT/2+250, 50, 10);
    insertAndSetFirstWall(&head, 6,  OVERALL_WINDOW_WIDTH/2-150, OVERALL_WINDOW_HEIGHT/2-100, 10, 100);
    insertAndSetFirstWall(&head, 7,  OVERALL_WINDOW_WIDTH/2-320, OVERALL_WINDOW_HEIGHT/2-230, 630, 10);
    insertAndSetFirstWall(&head, 8,  OVERALL_WINDOW_WIDTH/2-150, OVERALL_WINDOW_HEIGHT/2-100, 250, 10);
    insertAndSetFirstWall(&head, 9,  OVERALL_WINDOW_WIDTH/2+310, OVERALL_WINDOW_HEIGHT/2-230, 10, 440);
    insertAndSetFirstWall(&head, 10,  OVERALL_WINDOW_WIDTH/2+100, OVERALL_WINDOW_HEIGHT/2-100, 10, 200);
    insertAndSetFirstWall(&head, 11,  OVERALL_WINDOW_WIDTH/2+100, OVERALL_WINDOW_HEIGHT/2+210, OVERALL_WINDOW_WIDTH/2-100, 10);
    insertAndSetFirstWall(&head, 12,  OVERALL_WINDOW_WIDTH/2+200, OVERALL_WINDOW_HEIGHT/2+100, OVERALL_WINDOW_WIDTH/2-100, 10);
    insertAndSetFirstWall(&head, 13,  OVERALL_WINDOW_WIDTH/2+100, OVERALL_WINDOW_HEIGHT/2, 120, 10);
    insertAndSetFirstWall(&head, 14,  OVERALL_WINDOW_WIDTH/2, OVERALL_WINDOW_HEIGHT/2-190, 10, 55);
    insertAndSetFirstWall(&head, 15,  OVERALL_WINDOW_WIDTH/2-200, OVERALL_WINDOW_HEIGHT/2-165, 250, 10);
    insertAndSetFirstWall(&head, 16,  OVERALL_WINDOW_WIDTH/2+210, OVERALL_WINDOW_HEIGHT/2-230, 10, 150);
    //insertAndSetFirstWall(&head, 17,  OVERALL_WINDOW_WIDTH/2+190, OVERALL_WINDOW_HEIGHT/2-80, 40, 10);
    insertAndSetFirstWall(&head, 18,  OVERALL_WINDOW_WIDTH/2-200, OVERALL_WINDOW_HEIGHT/2+230, 300, 10);
    insertAndSetFirstWall(&head, 19,  OVERALL_WINDOW_WIDTH/2+100, OVERALL_WINDOW_HEIGHT/2+190, 10, 60);
    insertAndSetFirstWall(&head, 19,  OVERALL_WINDOW_WIDTH/2-250, OVERALL_WINDOW_HEIGHT/2-100, 10, 60);
    insertAndSetFirstWall(&head, 20,  OVERALL_WINDOW_WIDTH/2-320, OVERALL_WINDOW_HEIGHT/2-100, 80, 10);
    insertAndSetFirstWall(&head, 21,  OVERALL_WINDOW_WIDTH/2-320, OVERALL_WINDOW_HEIGHT/2-40, 80, 10);
    insertAndSetFirstWall(&head, 22,  OVERALL_WINDOW_WIDTH/2-320, OVERALL_WINDOW_HEIGHT/2+50, 280, 10);
    insertAndSetFirstWall(&head, 23, OVERALL_WINDOW_WIDTH/2-320, OVERALL_WINDOW_HEIGHT/2-230,10, 130);*/
    ...
    while(!done){
        ...
        if (!robot.crashed){
            ...
            //calls wallFinder() and saves a returned int as a movement code
            int m = wallFinderN(&robot, front_left_sensor, front_right_sensor, left_sensor, right_sensor);
            //checks the most recent movement against the one just before
            if (previousMove == m){
                moveCount++;
                //if the past 5 moves have been the same one, nudge() out of the loop
                if (moveCount > 5) {
                    nudge(&robot);
                    //printf("nudged\n");
                }
            } else {
                //this is a different move code, we aren't in a loop/same state
                //reset moveCount
                moveCount = 0;
                //update previousMove to the most recent one
                previousMove = m;
            }
        ...
        //Check if robot reaches endpoint. and check sensor values
        if (checkRobotReachedEnd(&robot, OVERALL_WINDOW_WIDTH, OVERALL_WINDOW_HEIGHT/2+100, 10, 100)){//ORIGINAL MAZE
        //if (checkRobotReachedEnd(&robot, OVERALL_WINDOW_WIDTH/2-320, OVERALL_WINDOW_HEIGHT/2-40, 10, 90)){OUR MAZE
        ...
        //Otherwise compute sensor information
            ...
            left_sensor = leftSensor(&robot, head);
            if (left_sensor > 0) {
                printf("Wall on left. Score = %d\n", left_sensor);
            }
            right_sensor = rightSensor(&robot, head);
            if (right_sensor > 0) {
                printf("Wall on right. Score = %d\n", right_sensor);
            }
